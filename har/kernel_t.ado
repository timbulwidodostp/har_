program define kernel_t, rclass
...
mat uadjomegahat=J(1,`nx',0)
mat uDFapprox=J(1,`nx',0)
mat uF=J(1,`nx',0)
mat ut=J(1,`nx',0)
mat ubopt=J(1,`nx',0)
mat uadjust=J(1,`nx',0)
...
mata: ukbmscore("`depva'","`indepv'","`instv'","`touse'","`RM'",`numr') 
...
/*cv from chi-squared distribution with df=p, p is the number of restriction */
...
/* calculate the F(t-square) statistic  */
mata: ulrvopt("`op'",`significance',`tau',`bmin',`bmax',`cv',`dlt2',"`score'","`thetahat'") 
  
mat uadjomegahat[1,`i']=sqrt(omegahat)
mat uDFapprox[1,`i']=DFapprox
mat uF[1,`i']=F
mat ut[1,`i']= sign(theta_iv[1,`i'])*sqrt(F)
mat ubopt[1,`i']=bopt
mat uadjust[1,`i']=adjust
  

}

return matrix uthetaiv=theta_iv
return matrix adjstderror=uadjomegahat
return matrix ut=ut
return matrix udf=uDFapprox
return matrix adjust=uadjust
return matrix bopt=ubopt
}
end


mata:
void udelta2(real scalar sig,
	 real scalar nre,
	 real scalar pow){
	 mm_root(x=.,&ufzxp(),0,25,0.01,1000,sig,nre,pow)
	 st_numscalar("udelta2",x)
	 }
     function ufzxp(
	 real scalar x,      
	 real scalar sig,
...
{
real scalar si,tao,bi,ba,cva,delta2,gq,q,c1,c2,T,d,MBbar,a1,a2,a,bopt,Ftemp,adjust,F,DFapprox
...
st_numscalar("omegahat",omegahat0)
}
end
